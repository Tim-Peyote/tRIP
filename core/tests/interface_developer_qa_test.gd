...
func _run() -> void:
	...
	var inventory_catalog := player.inventory.get_catalog(&"quality")
	_expect(inventory_catalog.size() == 1 and int(inventory_catalog[0]["specimen_count"]) == 2, "Inventory catalog lost its individual specimen records.")
	_expect(is_equal_approx(float(inventory_catalog[0]["average_freshness"]), 1.0), "Inventory catalog did not aggregate specimen freshness.")
	...
	_expect(hud.inventory_specimen_list.item_count == 2, "Inventory detail did not expose the two physical samples inside the stack.")
	...
	_expect(hud.journal_entry_list.item_count >= 1, "Herbarium did not create a selectable species card.")
	_expect(hud.journal_detail_title.text == "Лунная шляпка", "Herbarium did not open the selected species detail.")
