...
const OUTPUT_PATH: String = "/tmp/trip_counteragent_capture.png"
...
func _ready() -> void:
	...
	level.spore_tide.exposure = 0.18
	main.effect_orchestrator.apply_effects([&"effect.spore_quiet"], "Тлеющий контрагент")
	for _frame in 50:
		await get_tree().process_frame
	var image := get_viewport().get_texture().get_image()
	var error := image.save_png(OUTPUT_PATH)
	if error == OK:
		print("Counteragent capture saved: %s" % OUTPUT_PATH)
	main.queue_free()
	await get_tree().process_frame
	get_tree().quit(error)
