...
`ExpeditionObjectiveOrchestrator` принимает завершённые доменные события (`ItemInstance`, возвращение через portal), а не опрашивает инвентарь каждый кадр. `ExpeditionClock` публикует нормализованный progress и дискретную phase; chunk сам выбирает свою визуальную интерпретацию.
