...
Практическая карта редактируемых сцен, актуальная игровая композиция и ограничения миграции: [SCENE_EDITING.md](SCENE_EDITING.md).
...
## Player physiology boundary

`PlayerVitalsComponent` — единственный источник истины о физическом состоянии героя. Контроллер сообщает ему активность и запрашивает возможность бега/прыжка; погода сообщает влажность, ветер и температуру; эффекты сознания передают числовые gameplay channels; инвентарь сообщает факт употребления. Ни одна из этих систем не вычисляет итоговое здоровье или выносливость самостоятельно.

`ConsumableDefinition` содержит только data-driven метаболический профиль: группу замещения, длительность, прибавки ёмкости, регенерацию, тепло, сопротивления и токсичность. Три runtime-слота и их затухание принадлежат `PlayerVitalsComponent`. HUD читает snapshot и не хранит игровых значений. `SessionPersistenceOrchestrator` сохраняет snapshot через публичный save contract.

Такой контур позволяет позже добавлять травмы, одежду, навыки, сон и новые климатические воздействия отдельными источниками модификаторов, не разбрасывая проверки по контроллеру, погоде и интерфейсу.
